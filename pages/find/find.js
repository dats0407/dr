...
Page({
  data: {
    findArr: [],
    titleImg: "http://cisvideoqiniu.kazhedui.cn/cis/bolang/common/findtitlepic.png",
    needReload: false,
    needReloadFindId: 0,
    nowPage: 1,
    pageEnd: false,
  ...
  loadListData: function (callback) {
    //这一句是加载中，上一次加载未有返回，loadding变量未能重置
    //if (this.data.pageEnd == true || this.data.loadding == true) {
    
    if (this.data.pageEnd == true) {
      return;
    }
  ...
  deleteFind: function (findId) {
    ...
    AJAX.Request(config.findDeleteUrl, "GET", data, function (res) {
      var findArr = _this.data.findArr;
